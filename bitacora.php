...
$args = array(
	'post_type' => 'bitacora', 'posts_per_page' => -1,
	'tax_query' => array(
		array(
			'taxonomy' => 'proyectos',
			'field' => 'slug',			
...
while ($query->have_posts()) : $query->the_post(); 	
	...
	$imagen = featImg('full');
	$imagen = makeImg(timThumb($imagen,750,300 ));	
	$link = get_permalink($post->ID);
	$bitacora .= makeDiv("","entrada",
...
endwhile;
 	...
 	$bitacora = makeDiv("","eightcol last",$bitacora);

 	$echo .= makeDiv("","header twelvecol row",$lateral.$bitacora); 

 	echo makeDiv("bitacora","posts",$echo);
